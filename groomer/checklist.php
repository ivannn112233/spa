...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    foreach($items as $item){
        ...
        if($stmt->fetch()){
            // Actualizar
            $stmt = $db->prepare("
                UPDATE ficha_checklist 
                SET completado = :completado, observacion = :observacion 
                WHERE ficha_id = :ficha_id AND item_id = :item_id
            ");
        } else {
            // Insertar
            $stmt = $db->prepare("
                INSERT INTO ficha_checklist (ficha_id, item_id, completado, observacion) 
                VALUES (:ficha_id, :item_id, :completado, :observacion)
            ");
        }
        $stmt->execute([
    ...
    $stmt = $db->prepare("
        ...
        FROM ficha_checklist WHERE ficha_id = :ficha_id
    ");
    $stmt->execute([':ficha_id' => $cita['ficha_id']]);
    $stats = $stmt->fetch();
...
<head>
    ...
    <style>
        ...
        .checklist-item { border-left: 4px solid #3498db; margin-bottom: 15px; transition: all 0.3s; }
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST">
                        ...
                        ?>
                        <div class="checklist-item p-3 <?php echo $completado ? 'completed' : ''; ?>">
                        ...
                        <div class="mt-4">
                            ...
                            <?php if($porcentaje == 100): ?>
                                <a href="citas.php?cambiar_estado=<?php echo $cita_id; ?>&estado=completada" class="btn btn-success float-end" onclick="return confirm('¿Completar cita?')">
                                    <i class="fas fa-check-circle"></i> Finalizar Cita
                                </a>
                            <?php endif; ?>
                        </div>
                    </form>
