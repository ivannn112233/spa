...
if($filtro_usuario) $sql .= " AND a.usuario_id = '$filtro_usuario'";
if($filtro_accion) $sql .= " AND a.accion = '$filtro_accion'";
if($filtro_fecha) $sql .= " AND DATE(a.created_at) = '$filtro_fecha'";
if($filtro_rol) $sql .= " AND a.rol = '$filtro_rol'";
...
// Estadísticas
$stats = [];
$stmt = $db->query("SELECT COUNT(*) FROM audit_log"); $stats['total'] = $stmt->fetchColumn();
$stmt = $db->query("SELECT COUNT(*) FROM audit_log WHERE DATE(created_at) = CURDATE()"); $stats['hoy'] = $stmt->fetchColumn();
$stmt = $db->query("SELECT accion, COUNT(*) as total FROM audit_log GROUP BY accion ORDER BY total DESC LIMIT 5"); $stats['top_acciones'] = $stmt->fetchAll();

// Usuarios para filtro
$usuarios = $db->query("SELECT id, email FROM usuarios ORDER BY email")->fetchAll();

// Roles para filtro
$roles = $db->query("SELECT DISTINCT rol FROM audit_log WHERE rol IS NOT NULL")->fetchAll();
?>
<!DOCTYPE html>
<html lang="es">
<head>
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-3">
                    <div class="card bg-success text-white">
                        <div class="card-body">
                            <h3><?php echo number_format($stats['hoy']); ?></h3>
                            <p>Registros Hoy</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header bg-secondary text-white">Top 5 Acciones</div>
                        <div class="card-body">
                            <?php foreach($stats['top_acciones'] as $a): ?>
                            <div class="d-flex justify-content-between mb-1">
                                <span><?php echo $a['accion']; ?></span>
                                <span class="badge bg-primary"><?php echo $a['total']; ?></span>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    <form method="GET" class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <a href="auditoria.php" class="btn btn-secondary"><i class="fas fa-eraser"></i> Limpiar</a>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="card-body">
                    ...
                    <?php else: ?>
                            ...
                            <table class="table table-striped table-hover log-table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><small><?php echo htmlspecialchars(substr($log['detalle'] ?? '', 0, 100)); ?></small></td>
                        ...
                        <div class="mt-3 text-muted">
                            <small><i class="fas fa-info-circle"></i> Mostrando últimos <?php echo count($logs); ?> registros</small>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            
            <!-- Botón para limpiar logs (solo admin) -->
            <div class="card mt-4 bg-light">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <i class="fas fa-database"></i> <strong>Mantenimiento</strong>
                            <br><small class="text-muted">Eliminar registros antiguos (más de 3 meses)</small>
                        </div>
                        <button class="btn btn-danger btn-sm" onclick="limpiarLogs()" id="btnLimpiar">
